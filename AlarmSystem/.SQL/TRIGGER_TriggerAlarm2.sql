IF EXISTS (SELECT name FROM sysobjects WHERE name='TriggerAlarm2' AND type='TR')
DROP TRIGGER TriggerAlarm2
GO

CREATE TRIGGER TriggerAlarm2 ON TAGDATA
...
select @AlarmLowerLimit = AlarmLowerLimit from ALARMCONFIGURATION where AlarmConfigurationId=2
select @AlarmUpperLimit = AlarmUpperLimit from ALARMCONFIGURATION where AlarmConfigurationId=2
select @AlarmName = AlarmName from ALARMCONFIGURATION where AlarmConfigurationId=2
